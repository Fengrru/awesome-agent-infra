...
import { type SearchProvider, createSearchConnection } from "./connection/search.js"
...
import { GUIDANCE_WINDOW_EVENTS, buildGuidance, metacognize } from "./kernel/meta.js"
...
import { type Retriever, createRetriever } from "./memory/retriever.js"
...
export function createAgent(options: AgentOptions): Agent {
  ...
  const delegate = createDelegateConnection({
    runOne: (goal) => runDelegate(goal, options.workspace),
    runMany: (goals) =>
      Promise.all(
        goals.map((g) => {
          const dir = mkdtempSync(join(tmpdir(), "agent-worktree-"))
