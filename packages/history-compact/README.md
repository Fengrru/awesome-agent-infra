...
- `summarizeRound(events)` — model-free summary of one completed turn.
- `createPlanCompaction(reconstructHistory)` — returns a planner.
- `DEFAULT_COMPACT_THRESHOLD` — default 30,000 characters.
- `PlannedCompact` — `{ covers, events, summary }`.
