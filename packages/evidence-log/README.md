...
(the hand-written replacement for the original zod schema — this package has no
`zod` dependency).

## Querying

- `replay()` / `replaySince(ts)` — full or incremental history.
- `replaySession(sessionId)` — one session, ascending.
- `replayRecent(maxEvents)` — bounded tail for cheap statistics (guidance,
  meta-cognition) no matter how large the log grows.
- `eventsBefore(ts)` / `pruneBefore(ts)` — retention: archive then delete.
