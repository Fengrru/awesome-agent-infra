...
- `McpStdioClient` — spawns a server process, frames stdout into newline-delimited messages,
  drains stderr so a chatty server cannot deadlock, rejects pending requests on process exit.
- `McpHttpClient` — POSTs JSON-RPC to a single endpoint, captures `mcp-session-id`, sends
...
- Both implement `McpClient`:
  - `connect(): Promise<void>` — initialize + notifications/initialized + tools/list
  - `getTools(): McpTool[]`
...
**experimental** — API may change as the MCP protocol and this package evolve together.
