...
import type { McpCallResult, McpClient, McpHttpConfig, McpTool } from "./types.js"
...
/**
 * Model Context Protocol client over the streamable HTTP transport. JSON-RPC
 * requests are POSTed to a single endpoint; the server may reply with plain
 * JSON or with an SSE stream that stays open after the response (which is
 * parsed incrementally and abandoned as soon as the matching id arrives).
 */
