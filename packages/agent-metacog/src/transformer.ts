 ...
 * @module agent-metacog/transformer
 */

import type { CalibratorConfig, StreamFeatures } from "./calibrator-types"
import {
  addMatrices,
  dropMask,
  gelu,
...
export class SinusoidalPE {
  ...
  constructor(dim: number, maxLen = 2048) {
    this.dim = dim
    this.maxLen = maxLen
    this.cache = []
  }
  ...
  encode(seqLen: number): number[][] {
    ...
    for (let pos = 0; pos < seqLen; pos++) {
      if (pos < this.cache.length) {
        pe.push(this.cache[pos]!)
        continue
      }
      const row: number[] = new Array(this.dim)
      for (let i = 0; i < this.dim; i += 2) {
        const div = 10000 ** ((2 * Math.floor(i / 2)) / this.dim)
        const angle = pos / div
...
export class MultiHeadAttention {
  ...
  constructor(dModel: number, numHeads: number, dropoutRate = 0.1) {
    this.dModel = dModel
  ...
  private scaledDotProductAttention(
    ...
    for (let h = 0; h < numHeads; h++) {
      ...
      for (let i = 0; i < seqLen; i++) {
        const rowW = weights[i]!
        const rowOut = attended[i]!
        for (let t = 0; t < dK; t++) {
          let val = 0
  ...
  computeEntropy(attentionWeights: number[][][]): number[] {
    ...
    for (let h = 0; h < numHeads; h++) {
      ...
      for (let i = 0; i < seqLen; i++) {
        let hVal = 0
        const row = headWeights[i]!
        for (let j = 0; j < row.length; j++) {
          const p = row[j]!
          if (p > 0) hVal -= p * Math.log(p + 1e-12)
        }
        entropies[i] += hVal
      }
    }
...
export class TransformerLayer {
  ...
  private layerNorm(x: number[][], eps = 1e-6): number[][] {
    const seqLen = x.length
  ...
  private feedForward(x: number[][]): number[][] {
    ...
    for (let i = 0; i < seqLen; i++) {
      const row = hidden[i]!
      for (let j = 0; j < this.dFF; j++) row[j] = gelu(row[j]!)
    }
    ...
    if (this.dropoutRate > 0) {
      ...
      for (let i = 0; i < seqLen; i++) {
        const rowO = out[i]!
        const rowD = drop[i]!
        for (let j = 0; j < dim; j++) rowO[j] *= rowD[j]!
      }
    }
    return out
  }
...
export class MetacognitiveTransformer {
  ...
  likelProj: number[][]
  fusionProj: number[][]
  confidenceHead: number[][]
  difficultyHead: number[][]
  ...
  constructor(baseHiddenSize: number, config?: Partial<CalibratorConfig>) {
    ...
    this.layers = Array.from(
      { length: numLayers },
      () => new TransformerLayer(hiddenDim, numHeads, hiddenDim * 4, dropoutRate),
    )
  ...
  private reshapeFeatures(features: StreamFeatures): {
    semantic: number[][]
    attention: number[][]
    likelihood: number[][]
  } {
    ...
    const attention: number[][] = Array.from({ length: seqLen }, (_, i) =>
      scaleVector(attnRow, features.attentionEntropy[i]!),
    )
    const likelRow = this.likelProj[0]!
    const likelihood: number[][] = Array.from({ length: seqLen }, (_, i) =>
      scaleVector(likelRow, features.tokenLogLikelihoods[i]!),
    )
  ...
  private fuseStreams(features: StreamFeatures): number[][] {
    ...
    for (let i = 0; i < seqLen; i++) {
      const rowS = semProj[i]!
      const rowA = attention[i]!
      const rowL = likelihood[i]!
      const rowF = fused[i]!
      for (let j = 0; j < hiddenDim; j++)
        rowF[j] = ws.semantic * rowS[j]! + ws.attention * rowA[j]! + ws.likelihood * rowL[j]!
    }
    return matMul(fused, this.fusionProj)
  }

  temperatureScale(logits: number[], temp: number): number[] {
