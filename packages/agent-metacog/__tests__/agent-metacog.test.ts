...
describe('AgentMetacog', () => {
  ...
  test('evaluateMetacognition returns full state', () => {
    ...
    metacog.recordInteraction({
      domain: 'math', query: 'q', success: true, timestamp: new Date(), selfConfidence: 0.9,
    });
    metacog.recordInteraction({
      domain: 'physics', query: 'q', success: false, timestamp: new Date(), selfConfidence: 0.8, failureReason: 'wrong formula',
    });
  ...
  test('interaction without failure does not create gap', () => {
    ...
    metacog.recordInteraction({
      domain: 'math', query: 'q', success: false, timestamp: new Date(),
      // no failureReason
    });
