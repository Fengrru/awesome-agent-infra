...
export class EngineDatabase {
  ...
  countEvents(sessionId: string): number {
    if (!this.db) return 0
    const row = this.db.query(`SELECT COUNT(*) as c FROM ${T.EVENT_LOG} WHERE session_id = ?`).get(sessionId) as {
      c: number
    } | null
    return row?.c ?? 0
  }
  ...
  deleteColdEvents(sessionId: string, maxSeqIndex: number, archivePath: string, archivedCount: number): void {
    if (!this.db) return
    this.db.transaction(() => {
      this.db!.run(`DELETE FROM ${T.EVENT_LOG} WHERE session_id = ? AND sequence_index <= ?`, sessionId, maxSeqIndex)
      this.db!.run(
  ...
  insertCheckpoint(cp: Checkpoint): void {
    ...
    this.db.run(
      ...
      cp.last_event_id,
      cp.level,
      JSON.stringify(cp.execution_state),
      cp.context_hash,
      cp.git_head_hash ?? null,
      cp.created_at,
    )
  }
  ...
  getLatestCheckpoint(sessionId: string, level?: string): Checkpoint | null {
    ...
    const row = (level ? this.db.query(query).get(sessionId, level) : this.db.query(query).get(sessionId)) as Record<
      string,
      unknown
    > | null
    if (!row) return null
  ...
  getCapabilities(): Capability[] {
    if (!this.db) return []
    const rows = this.db.query(`SELECT * FROM ${T.CAPABILITY} ORDER BY success_rate DESC`).all() as Record<
      string,
      unknown
    >[]
    return rows.map((r) => ({
  ...
  getMemories(sessionId: string): LongTermMemory[] {
    if (!this.db) return []
    const rows = this.db
      .query(`SELECT * FROM ${T.MEMORY} WHERE session_id = ? OR session_id = 'global' ORDER BY retention_score DESC`)
      .all(sessionId) as Record<string, unknown>[]
    return rows.map((r) => ({
  ...
  searchByTags(tags: string[]): LongTermMemory[] {
    ...
    const rows = this.db
      .query(`SELECT * FROM ${T.MEMORY} WHERE (${likeClauses}) ORDER BY retention_score DESC`)
  ...
  upsertSession(session: {
    ...
    this.db.run(
      ...
      session.current_checkpoint_id ?? null,
      now,
      now,
    )
  }
  ...
  getSession(sessionId: string): Record<string, unknown> | null {
    if (!this.db) return null
    return this.db.query(`SELECT * FROM ${T.SESSION} WHERE session_id = ?`).get(sessionId) as Record<
      string,
      unknown
    > | null
  }
  ...
  getAgentSelfRules(): CoreRule[] {
    if (!this.db) return []
    const rows = this.db.query(`SELECT * FROM ${T.AGENT_SELF} ORDER BY importance DESC`).all() as Record<
      string,
      unknown
    >[]
    return rows.map((r) => ({
  ...
  getUserProfiles(userHash?: string): Array<{
    ...
    const rows = userHash
      ? (this.db
          .query(`SELECT * FROM ${T.USER_PROFILE} WHERE user_hash = ? ORDER BY importance DESC`)
...
/**
 * Create a {@link EngineDatabase} instance.
 *
 * @param args - Constructor arguments forwarded to {@link EngineDatabase}.
 * @returns A new {@link EngineDatabase}.
 */
export function createEngineDatabase(...args: ConstructorParameters<typeof EngineDatabase>): EngineDatabase {
  return new EngineDatabase(...args)
}
