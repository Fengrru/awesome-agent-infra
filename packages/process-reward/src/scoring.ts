...
import type {
  TaskType, MathHeuristicOptions, CodeHeuristicOptions,
  LogicHeuristicOptions, HeuristicConfig, GenerateFn, VerifyFn,
} from "./types"
import {
  DEFAULT_MATH_OPTIONS, DEFAULT_CODE_OPTIONS, DEFAULT_LOGIC_OPTIONS,
} from "./types"
...
export function scoreMathStep(
  step: string, previousStep: string | null,
  ...
  if (/\b(substitute|plug|replace)\b/i.test(step) || /\bsince\s+\w+\s*=\s*\d+(?:\.\d+)?/i.test(step) ||
  ...
  if (previousStep) {
    ...
    if (prevNums.length > 0 && currNums.length > 0) {
      const prevMag = prevNums.map(n => Math.log10(Math.abs(parseFloat(n)) + 1e-9))
      const currMag = currNums.map(n => Math.log10(Math.abs(parseFloat(n)) + 1e-9))
...
export function scoreCodeStep(
  step: string, _previousStep: string | null,
  options: CodeHeuristicOptions = DEFAULT_CODE_OPTIONS,
): number {
  let score = 0.5
  const structural = [/\b(def|class|function|const|let|var|import|export|from|return)\b/,
    /\b(if|elif|else|for|while|switch|case)\b/, /\b(try|catch|finally|throw|raise)\b/,
    /\b(async|await|Promise|yield)\b/, /\b(type|interface|enum|implements|extends)\b/]
  ...
  const dangerous = [/\bos\.system\b/i, /\bsubprocess\b/i, /\bexec\s*\(/i, /\beval\s*\(/i,
    /\brm\s+-rf\b/, /\bshutil\.rmtree\b/i, /\bos\.remove\b/i, /\bimport\s+ctypes\b/i,
    /\b__import__\s*\(/, /\bsocket\b/i]
...
export function scoreLogicStep(
  ...
  if (/[→⇒⟹∨∧¬∀∃⊢⊨]/.test(step) || /\bimplies\b|\band\s+also\b|\bsuch\s+that\b/i.test(step)) score += options.premiseWeight * 0.8
  if (/\b(contrary|opposite|negation|for\s+contradiction|suppose\s+not)\b/i.test(step)) score += options.conclusionWeight * 1.2
  if (/\b(therefore|thus|hence|so|consequently|it\s+follows)\b/i.test(step) || /\bQ\.?E\.?D\.?\b/i.test(step)) score += options.conclusionWeight
  ...
  if (previousStep && /\b(assume|let|given|suppose|consider)\b/i.test(previousStep) &&
...
export function heuristicScore(
  step: string, previousStep: string | null,
  taskType: TaskType, config?: HeuristicConfig,
): number {
  switch (taskType) {
    case "math": return scoreMathStep(step, previousStep, { ...DEFAULT_MATH_OPTIONS, ...config?.math })
    case "code": return scoreCodeStep(step, previousStep, { ...DEFAULT_CODE_OPTIONS, ...config?.code })
    case "logic": return scoreLogicStep(step, previousStep, { ...DEFAULT_LOGIC_OPTIONS, ...config?.logic })
    default: return scoreGeneralStep(step, previousStep)
  }
}
...
export async function mcRolloutLabel(
  steps: string[], stepIndex: number, referenceAnswer: string,
  generateFn: GenerateFn, verifyFn: VerifyFn, numRollouts: number,
