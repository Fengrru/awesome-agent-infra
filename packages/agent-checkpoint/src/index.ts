...
export interface L1Snapshot {
  ...
  dag_progress: {
    version: number; total_nodes: number; completed_nodes: number; failed_nodes: number
...
export interface MessageRef {
  event_id: string; sequence_index: number; summary: string; token_count: number
}

export interface FileContextRef {
  file_path: string; content_hash: string; relevant_lines: [number, number]; summary: string
}
...
export interface MemoryPointer {
  memory_id: string; memory_type: "session" | "agent_self" | "user_profile"; relevance_score: number
}
...
export interface SessionMeta {
  title: string; goal: string; total_events: number; total_tokens: number
  duration_ms: number; created_at: number; completed_at?: number
}

export interface Checkpoint {
  checkpoint_id: string; session_id: string; last_event_id: string
  level: "L1" | "L2" | "L3"
...
class LRUCheckpointCache {
  ...
  get(id: string): Checkpoint | undefined {
    ...
    if (entry) { this.map.delete(id); this.map.set(id, entry); return entry.cp }
    return undefined
  }

  set(id: string, cp: Checkpoint, sizeBytes: number, onEvict?: (cp: Checkpoint) => void): void {
    const old = this.map.get(id)
    if (old) { this.totalBytes -= old.sizeBytes; this.map.delete(id) }
    while (this.totalBytes + sizeBytes > this.maxBytes && this.map.size > 0) {
      const firstKey = this.map.keys().next().value
      if (!firstKey) break
      const evicted = this.map.get(firstKey)!
      this.totalBytes -= evicted.sizeBytes; this.map.delete(firstKey)
      onEvict?.(evicted.cp)
    }
    this.map.set(id, { cp, sizeBytes }); this.totalBytes += sizeBytes
  }

  has(id: string): boolean { return this.map.has(id) }
  delete(id: string): void { const e = this.map.get(id); if (e) { this.totalBytes -= e.sizeBytes; this.map.delete(id) } }
  clear(): void { this.map.clear(); this.totalBytes = 0 }
  get size(): number { return this.map.size }
  get memoryBytes(): number { return this.totalBytes }
}
...
export class CheckpointManager {
  ...
  private readonly MAX_L1 = 10; private readonly MAX_L2 = 5; private readonly MAX_L3 = 20
  ...
  private l1Cps: Checkpoint[] = []; private l2Cps: Checkpoint[] = []; private l3Cps: Checkpoint[] = []

  constructor() { this.lruCache = new LRUCheckpointCache(5 * 1024 * 1024) }

  setDatabase(db: CheckpointDatabase): void { this.db = db }

  private persist(cp: Checkpoint): void { if (this.db?.isConnected()) this.db.insertCheckpoint(cp) }

  private onCacheEvict = (cp: Checkpoint): void => { this.persist(cp) }
  ...
  createL1(sessionId: string, snapshot: L1Snapshot, contextHash: string, lastEventId: string): Checkpoint {
    ...
    const cp: Checkpoint = {
      checkpoint_id: `cp_${Date.now()}_L1`, session_id: sessionId, last_event_id: lastEventId,
      level: "L1", execution_state: snapshot as unknown as Record<string, unknown>,
      context_hash: contextHash, created_at: Date.now(),
    }
  ...
  createL2(sessionId: string, snapshot: L2Snapshot, contextHash: string, gitHeadHash: string, lastEventId: string): Checkpoint {
    const sizeBytes = this.getCheckpointSize(snapshot)
    this.validateSize("L2", sizeBytes)
    const cp: Checkpoint = {
      checkpoint_id: `cp_${Date.now()}_L2`, session_id: sessionId, last_event_id: lastEventId,
      level: "L2", execution_state: snapshot as unknown as Record<string, unknown>,
      context_hash: contextHash, git_head_hash: gitHeadHash, created_at: Date.now(),
    }
  ...
  createL3(sessionId: string, snapshot: L3Snapshot, contextHash: string): Checkpoint {
    ...
    const cp: Checkpoint = {
      checkpoint_id: `cp_${Date.now()}_L3`, session_id: sessionId, last_event_id: "",
      level: "L3", execution_state: snapshot as unknown as Record<string, unknown>,
      context_hash: contextHash, created_at: Date.now(),
    }
  ...
  getLatest(sessionId: string, level?: "L1" | "L2" | "L3"): Checkpoint | null {
    ...
    return [...this.l1Cps, ...this.l2Cps, ...this.l3Cps].filter(filterBySession).at(-1)
      ?? this.db?.getLatestCheckpoint(sessionId) ?? null
  }
  ...
  validateSize(level: "L1" | "L2" | "L3", sizeBytes: number): { ok: boolean; sizeBytes: number; limitBytes: number; warning?: string } {
    ...
    if (sizeBytes > limitBytes) {
      return { ok: false, sizeBytes, limitBytes, warning: `${level} checkpoint ${(sizeBytes/1024).toFixed(1)}KB exceeds ${(limitBytes/1024).toFixed(0)}KB limit` }
    }
    return { ok: true, sizeBytes, limitBytes }
  }

  getAllCheckpoints(): Checkpoint[] { return [...this.l1Cps, ...this.l2Cps, ...this.l3Cps] }
  getCacheMemoryBytes(): number { return this.lruCache.memoryBytes }
  clear(): void { this.l1Cps = []; this.l2Cps = []; this.l3Cps = []; this.lruCache.clear() }
}
