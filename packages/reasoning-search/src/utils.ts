...
export function adaptiveFloor(depth: number, discountFactor: number): number {
  return 1e-4 * Math.pow(discountFactor, depth);
}
...
export function isComplete(text: string, taskType: SearchTaskType): boolean {
  switch (taskType) {
    case 'math': return isCompleteMath(text);
    case 'code': return isCompleteCode(text);
    case 'logic': return isCompleteLogic(text);
 ...
 *   IMPLICATION → ASSERTION → EQUATION → CONCLUSION → UNKNOWN
 */
