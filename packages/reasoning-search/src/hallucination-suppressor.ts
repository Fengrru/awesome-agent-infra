import type { FactualClaim, VerifiedClaim, HallucinationReport } from "./utils"
...
export class HallucinationSuppressor {
  ...
  private static readonly VERIFICATION_TEMPLATES = [
    ...
    (claim: string) =>
      `Fact-check this statement and respond with only "YES" (correct) or "NO" (incorrect):\n${claim}`,
  ];
  ...
  async suppress(text: string): Promise<HallucinationReport> {
    ...
    const overallConfidence =
      claims.length > 0 ? verifiedCount / claims.length : 1.0;
