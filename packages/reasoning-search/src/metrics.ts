...
export class MetricCalculator {
  ...
  static compute(
    ...
    for (let i = 0; i < N; i++) {
      const pred = predictions[i]!.trim().toLowerCase().replace(/\s+/g, " ")
      const ref = references[i]!.trim().toLowerCase().replace(/\s+/g, " ")
    ...
    const bleu = MetricCalculator.corpusBleu(predictions.slice(0, N), references.slice(0, N))

    // ── ECE (Expected Calibration Error) ──
    let ece = 0
    if (confidences.length >= N && N > 0) {
      const corrects = predictions.map((p, i) => (p.trim() === references[i]?.trim() ? 1 : 0))
    ...
    const avgTime = timesMs.length > 0 ? timesMs.reduce((a, b) => a + b, 0) / timesMs.length : 0
  ...
  private static corpusBleu(predictions: string[], references: string[]): number {
    ...
    for (let weight = 1; weight <= 4; weight++) {
      ...
      for (let i = 0; i < predictions.length; i++) {
        ...
        weightedPrecision += predNgrams.length > 0 ? Math.log(matchCount / predNgrams.length) : 0
    ...
    // Brevity penalty
    const bp = totalPredLen < totalRefLen ? Math.exp(1 - totalRefLen / Math.max(totalPredLen, 1)) : 1.0
  ...
  private static computeECE(confidences: number[], corrects: number[], numBins: number): number {
