...
let nodeId = 0
function createNode(state: string, action: string | null, parent: MCTSNode | null): MCTSNode {
  return {
    id: `test_${++nodeId}`,
    state,
    action,
...
describe("ReasoningSearch", () => {
  test("standard_sampling returns single generation", async () => {
    const engine = new ReasoningSearch(mockGenerate(["The answer is 42."]))
  ...
  test("mcts search returns valid result", async () => {
    const engine = new ReasoningSearch(
      mockGenerate(["Step 1: Note that 6*7 = 42", "Step 2: Therefore the answer is 42", "#### 42"]),
  ...
  test("mcts search with score function", async () => {
    ...
    const engine = new ReasoningSearch(mockGenerate(["x = 5", "x + 3 = 8", "#### 8"]), {
      scoreFn,
      config: { mctsIterations: 15, maxDepth: 8, beamWidth: 3 },
    })
  ...
  test("guided_beam_search returns valid result", async () => {
    const engine = new ReasoningSearch(mockGenerate(["Step 1: analyze", "Step 2: conclude #### 7"]), {
  ...
  test("importance_sampling returns valid result", async () => {
    const engine = new ReasoningSearch(mockGenerate(["Step A", "Step B", "Step C"]), {
  ...
  test("extractBestPath returns at least root", async () => {
    const engine = new ReasoningSearch(mockGenerate(["answer"]), {
  ...
  test("best_of_n search returns valid result", async () => {
    const engine = new ReasoningSearch(mockGenerate(["The answer is 42", "I think 42", "42"]), {
      config: { maxDepth: 10, beamWidth: 2 },
    })
  ...
  test("legacy_beam_search returns valid result", async () => {
    const engine = new ReasoningSearch(mockGenerate(["Step 1: setup", "Step 2: solve", "#### 42"]), {
  ...
  test("mcts with task type code", async () => {
    const engine = new ReasoningSearch(mockGenerate(["def solve():", "    return 42"]), {
  ...
  test("mcts with logic task type", async () => {
    const engine = new ReasoningSearch(mockGenerate(["Assume P is true.", "Therefore Q must be true.", "QED"]), {
  ...
  test("guided_beam_search with score function weights steps", async () => {
    ...
    const engine = new ReasoningSearch(mockGenerate(["good step", "bad step", "good step 2"]), {
      scoreFn,
      config: { maxDepth: 5, beamWidth: 2, prmBeta: 2.0 },
    })
  ...
  test("config override in solve merges with defaults", async () => {
    const engine = new ReasoningSearch(mockGenerate(["answer"]), { config: { maxDepth: 20 } })
...
describe("HallucinationSuppressor", () => {
  ...
  test("extractClaims splits on sentence boundaries", () => {
    ...
    const claims = suppressor.extractClaims("The sky is blue. The earth is round. Water boils at 100 degrees.")
  ...
  test("suppress pipeline works end-to-end", async () => {
    ...
    const report = await suppressor.suppress("The sky is blue. Water is wet. The earth is flat.")
  ...
  test("suppress removes hallucinated claims from corrected text", async () => {
    ...
    const report = await suppressor.suppress("The earth is flat. Water is wet.")
...
describe("MetricCalculator", () => {
  ...
  test("perfect exact match", () => {
    const result = MetricCalculator.compute(["hello world", "foo bar"], ["hello world", "foo bar"])
  ...
  test("no match", () => {
    const result = MetricCalculator.compute(["abc def", "ghi jkl"], ["xyz uvw", "rst mno"])
    expect(result.exactMatch).toBe(0)
    expect(result.partialMatch).toBe(0)
  })

  test("partial match (substring)", () => {
    const result = MetricCalculator.compute(["hello world today"], ["hello world"])
    expect(result.partialMatch).toBe(1.0)
  })

  test("partial match reversed", () => {
    const result = MetricCalculator.compute(["hello"], ["hello world"])
    expect(result.partialMatch).toBe(1.0)
  })

  test("ROUGE scores", () => {
    const result = MetricCalculator.compute(["the cat sat on the mat"], ["the cat sat on the mat"])
    expect(result.rouge1).toBeCloseTo(1.0)
    expect(result.rouge2).toBeCloseTo(1.0)
  })

  test("BLEU score for identical strings", () => {
    const result = MetricCalculator.compute(["the quick brown fox"], ["the quick brown fox"])
    expect(result.bleu).toBeGreaterThan(0.9)
  })

  test("ECE with confidences", () => {
    const result = MetricCalculator.compute(["a", "b", "c"], ["a", "b", "c"], [0.9, 0.9, 0.9])
    expect(result.ece).toBeLessThan(1.0)
  })

  test("efficiency metrics with call counts and times", () => {
    const result = MetricCalculator.compute(["a", "b"], ["a", "b"], [], [5, 3], [100, 200])
  ...
  test("mismatched lengths use minimum", () => {
    const result = MetricCalculator.compute(["a", "b", "c"], ["a"])
    expect(result.numSamples).toBe(1)
  })

  test("case-insensitive exact match", () => {
    const result = MetricCalculator.compute(["HELLO"], ["hello"])
    expect(result.exactMatch).toBe(1.0)
  })

  test("whitespace differences still match", () => {
    const result = MetricCalculator.compute(["hello  world"], ["hello world"])
